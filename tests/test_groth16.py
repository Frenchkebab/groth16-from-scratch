...
def test_verify(groth16Verifier_contract):
    ...
    U, V, W, a, Ua, Va, Wa, h, t = get_qap(x,y)

    powers_of_tau_A, powers_of_tau_B, powers_of_tau_C, powers_of_tau_HT, alpha1, beta2 = trusted_setup(U, V, W, t, Ua.degree)

    A1 = inner_product(powers_of_tau_A, Ua.coeffs[::-1], alpha1)
    B2 = inner_product(powers_of_tau_B, Va.coeffs[::-1], beta2)
    C1_prime = inner_product(powers_of_tau_C, Wa.coeffs[::-1], Z1) # C_1'
    ...
    assert groth16Verifier_contract.verify(A1_G1.getPoint(), B2_G2.getPoint(), C1_G1.getPoint())
